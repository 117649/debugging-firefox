...
| Agent host | Hosts that support the [Agent Skills specification](https://agentskills.io/specification) | No host-specific installation or runtime acceptance is recorded here. |
...
Live work requires an explicitly selected executable and loopback port, one mutation owner, one task-owned socket, a captured baseline, and a restoration plan. Restart additionally requires separate authorization and release of other mutation owners. If any prerequisite is absent, stop and report the unverified boundary.

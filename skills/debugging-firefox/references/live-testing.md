...
Independently prove a real browser-chrome window with platform window enumeration or automation correlated to the retained process and selected executable/profile. On Windows require a visible, non-cloaked top-level Firefox browser window; process presence, launcher PID/exit, a nonzero `MainWindowHandle`, a blank-title/hidden window, helper success, and listener appearance are not proof.

...
| Window enumeration unavailable or inconclusive for a selected-target process | Preserve pre-existing Firefox and listener state; retain a task-owned cold launch through its bounded window wait. Make exactly one fresh list-only recovery attempt: prefer a read-only subagent when it provides a fresh tool service/session; otherwise use one fresh local service/session, never both. If it cannot return a trusted result, stop as `window evidence unavailable`; clean a causally proven task-owned cold-launch tree only after revalidating its identity, exclusive current ownership, and release by other tasks, then verify its selected port remains closed. Otherwise leave and report it. Do not invoke the listener, restart pre-existing Firefox, or call this no-window proof. |

...
Classify in this order: existing listener, target identity, real browser-window state, then launch branch. Do not invoke the debugger flag until the selected target fits one row:
...
| No selected-target process and no listener | Cold start: launch the selected executable normally with no debugger flag or invented profile/isolation arguments. Causally correlate the retained tree to this launch by baseline delta, exact executable/profile, start time, retained-instance identity, and lineage where available; a delta alone is not ownership. Wait boundedly for a real browser window, then make the separate debugger-server invocation. A combined cold debugger-server launch is forbidden. |
| Window enumeration unavailable or inconclusive | Preserve pre-existing Firefox and listener state; retain a task-owned cold launch through its bounded window wait. Make exactly one fresh list-only recovery attempt: prefer a read-only subagent when it provides a fresh tool service/session; otherwise use one fresh local service/session, never both. If it cannot return a trusted result, stop as `window evidence unavailable`; clean only a causally proven task-owned cold-launch tree and verify its selected port remains closed. Do not invoke the listener, restart pre-existing Firefox, or call this no-window proof. |
| Successful trusted enumeration proves selected-target processes but no real browser window | This is neither cold start nor existing-window attachment. Do not invoke the debugger server. If this task just made the normal launch, finish its bounded window wait and clean only its proven tree on timeout; otherwise stop and leave all processes untouched. |
...
When a window tool fails before enumeration, make exactly one recovery attempt. Prefer one fresh read-only subagent when it provides a separate tool service/session; otherwise reinitialize one fresh local service/session. Do not try both sequentially. Issue only a list/state query. On Codex Computer Use, when `@oai/sky` is already available, the bounded recovery call is:
...
A separate subagent service prevents a stale parent-task helper from ending the Firefox task. A repeated `failed to write kernel assets ... (os error 3)` after `js_reset` is still a pre-enumeration helper failure: do not loop reset, recreate a guessed temporary directory, kill shared helper processes, or restart Firefox. If the one fresh service/session also fails, report `window evidence unavailable`; a full agent-host relaunch is a later user-controlled fallback after active tasks finish. A screenshot policy error after successful listing does not invalidate the listing.
...
On Windows pass the exact executable as `Start-Process -FilePath` and its tokens through `-ArgumentList`. The result or exited PID may be only the command-line forwarding helper. After the separate debugger invocation, take bounded error-visible listener evidence, resolve its owning Firefox process and retained instance when possible, then follow the recipe below. An empty error-suppressed port query is not evidence.

## Listener evidence, logs, and connection attempts

Count the listener request separately from client attempts. Invoke the selected executable with `--start-debugger-server PORT` exactly once. Never replay that request because a port enumerator is empty or a client is refused.
...
[`DevToolsStartup.handleDevToolsServerFlag`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_154_0_RELEASE/devtools/startup/DevToolsStartup.sys.mjs#L1117-L1194) calls `listener.open()` without awaiting its result and immediately dumps `Started devtools server on PORT`; [`SocketListener.open`](https://github.com/mozilla-firefox/firefox/blob/FIREFOX_154_0_RELEASE/devtools/shared/security/socket.js#L393-L441) reports a rejected open as `Could not start debugging listener on 'PORT': ...`. The first line proves the handler reached the open request, not that binding ultimately succeeded. A matching second line is explicit open failure. Missing lines are inconclusive.

The first real RDP attempt is mandatory and is not a retry. Inspect `FirefoxRdpClient.tcpAccepted` after failure. When it is `false`, dispose that client and make at most one sequential retry only if the listener was just requested by this task, the same bounded readiness deadline remains, target/window/ownership evidence is unchanged, OS/log refresh shows no explicit open failure or unsafe/foreign bind, and no task operation ran. Use a new client with an ephemeral local port; do not replay the listener request. A second pre-accept failure marks the listener unhealthy. When `tcpAccepted` is `true`, keep that socket for approval and capability preflight: no retry or reconnect after acceptance.
...
| Task cold-started dedicated Firefox | Close the client, terminate only the causally proven task-owned retained process tree, and verify both the tree and its listener stopped. Incomplete proof leaves Firefox running. |
